...
def cifrarCRTbasica(paquetes):  # Funcion para el calculo de las simulaciones y almacenamiento de
    # resultados parciales, que no incluye los descifrados ni el cifrado completo de
    # nuestra propuesta
    #MSDUs = AMSDU_gen(ndatip, tdatip)
    
    for i in range(2):
        x=int.from_bytes(paquetes[i][0], byteorder='big')
        print("MSDU:", x)
        print(hexdump(x.to_bytes((x.bit_length() + 7) // 8, byteorder='big')))

    (Hdr, mpduCi, ps, xs) = AMSDU_enc(paquetes) #para mas adelante, añadir las claves como parametros de entrada y quitarlos de la salida
...
def AMSDU_enc(paquetes):  # Función que cifra un AMSDU con el teorema chino del resto (CRT) - modificacion con poner un AMPDU
    ...
    for paquete in paquetes:
        ...
        i = int.from_bytes(paquete[0], byteorder='big')
        ...
        a.append((i + pow(Hdr, xs, ps1)) % ps1)  # y enmascaro el MSDU
        n= n +1
