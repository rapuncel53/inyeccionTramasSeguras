...
def cifrarCRTbasica(ndatip,tdatip):  # Funcion para el calculo de las simulaciones y almacenamiento de
# resultados parciales, que no incluye los descifrados ni el cifrado completo de
# nuestra propuesta

    MSDUs = AMSDU_gen(ndatip, tdatip)
    for i in range (ndatip):
    	print("MSDU:",i+1, MSDUs[i])
    	print(hexdump(MSDUs[i].to_bytes((MSDUs[i].bit_length() + 7) // 8, byteorder='big')))
 
    (Hdr, mpduCi, ps, xs) = AMSDU_enc(MSDUs) #para mas adelante, añadir las claves como parametros de entrada y quitarlos de la salida
...
def AMSDU_enc(MSDU):  # Función que cifra un AMSDU con el teorema chino del resto (CRT) - modificacion con poner un AMPDU
